...
def train(loader, lstmDec, linNet, lstmEnc, LM, crit, optimizer, savepath):
	...
	while True:
		...
		for i in qdar:
			...
			qdar.set_postfix(tau=str(tau),lmLoss=lstr(loss2))
			if i > 0 and i % 1000 == 0:
...
if __name__ == '__main__':
	...
	LM =  LanguageModelLoss(
		wordDict=VocabData['word_dict'], PATH="./data/LMcheckpoint", vocab_size=len(VocabData['word_dict']),max_len=15,sos_id=sos_id, eos_id=eos_id , embedding_size=300,hidden_size=1024, use_prob_vector=False
	)
